window.vu.collections = {

/////////////////////////////////////////////////////////////////////////////}
/// COLLECTIONS DECLARATION /////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////{
	// Now let's define a new Collection of Events
	EventCollection : Backbone.Collection.extend({
		// The couchdb-connector is capable of mapping the url scheme
		// proposed by the authors of Backbone to documents in your database,
		// so that you don't have to change existing apps when you switch the sync-strategy
		url : "event",
		model : window.vu.models.EventModel,
		// The events should be ordered by date
		comparator : function(event){
			return new Date( event.get("date") ).getTime();
		}
	}),

	BandCollection : Backbone.Collection.extend({
		url : "band",
		model : window.vu.models.BandModel,
		comparator : function(band){
			return band.get("bandName");
		}
	}),

	HallCollection : Backbone.Collection.extend({
		url : "dancehall",
		model : window.vu.models.VenueModel,
		comparator : function(hall){
			return hall.get("danceHallName");
		}
	})
};

	