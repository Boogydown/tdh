...
(function( $ ){
	...
	$.extend($.ui, { multiDatesPicker: { version: "1.5.1" } });
	...
	$.fn.multiDatesPicker = function(method) {
		...
		var methods = {
			init : function( options ) {
				...
				if (altFieldOption) $(altFieldOption).val('');
			},
			...
			dateConvert : function( date, desired_format, dateFormat ) {
				...
				if(!dateFormat) {
					...
					if ($this.datepicker('option', 'dateFormat') != undefined && $this.datepicker('option', 'dateFormat') != "") {
			...
			toggleDate : function( date, type ) {
				...
				switch(mode.modeName) {
					case 'daysRange':
						...
						break;
					case 'abDaysRange':
						date = methods.dateConvert( date, "object" );
						...
						// prevent spanning over into another month; will just select single date
						if ( begin && date.getMonth() != begin.getMonth() )
							begin = end = null;
						...
						else // date is inside current range, so reset view to just one
							begin = end = date;
